...
$PASSWORD = 'pasporty';  // Set the password, to access the file manager... (optional)
...
if($PASSWORD) {
	...
	if(!$_SESSION['_sfm_allowed']) {
		...
		echo '<html><body><form action=? method=post>HESLO:<input type=password name=p autofocus/></form></body></html>';
		exit;
	}
}
...
<?php endif; ?>
	...
	function renderFileRow(data) {
		...
		var $dl_link = $('<a/>').attr('href','?do=download&file='+ encodeURIComponent(data.path))
			.addClass('download').text('stáhnout');
		var $delete_link = $('<a href="#" />').attr('data-file',data.path).addClass('delete').text('smazat');
		var perms = [];
	...
	function renderBreadcrumbs(path) {
		var base = "",
			$html = $('<div/>').append( $('<a href=\"javascript: history.go(-1)\">ZPĚT</a></div>') );
...
<footer>simple php filemanager by <a href="https://github.com/jcampbell1">jcampbell1</a>; upravil Jan ROLLER</footer>
</body></html>
